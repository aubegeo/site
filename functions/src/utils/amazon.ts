export function formatAmazonAffiliateLink(link: string, productName: string, affiliateTag = 'autogeo07-21') {
    try {
      ...
      if (!url.hostname.includes('amazon.')) return link
  
      // Vérifie s'il y a déjà un tag affilié Amazon
      if (url.searchParams.has('tag')) return link
